...
- 完成优化：
  - 重写 `README.md`，补齐产品定位、已支持能力、技术栈、目录结构、本地运行、环境变量、验证命令、生产部署清单、微信公众号接入方向和后续拓展优先级。
  - 明确当前边界：不做完整账号体系、站内私信、模板拖拽、小程序和复杂地图供应商适配。
  - 增加 README 静态测试，防止后续删掉关键交接信息。
- 验证记录：
  - `npm test -- tests/readme-project-docs.test.mts` 通过。

## 2026-06-17 定位回填、标志物折叠与实时预览

- 完成用户反馈：
  - 点击“使用当前位置”后，前端先保存 `lat/lng`，再调用后端 `/api/location/reverse-geocode` 反查地址。
  - 反查成功时自动回填省/市/区/街道、详细地址、地点名称和 `regionCode`，用户只需要检查和补充楼栋/门口等细节。
  - 新增 `REVERSE_GEOCODE_ENDPOINT` 和 `REVERSE_GEOCODE_EMAIL` 环境变量，默认使用 Nominatim；生产建议替换成可商用地图服务。
  - “附近标志物”默认收起成 `+ 添加附近标志物`，点击后再展示输入框，减少首步视觉负担。
  - 创建流程增加“实时预览”卡片，边输入边看到海报中会显示的名称、宠物类型、地点和联系方式。
  - `/mine` 页面顶部从高 hero 改为紧凑页头，减少“我的启事”占用高度。
- TDD 记录：
  - 先新增逆地理编码映射测试和 API route 静态测试，确认服务缺失时失败。
  - 先新增表单 UI 静态测试，确认反查调用、标志物折叠、实时预览和紧凑页头缺失时失败。
  - 再实现后端反查服务、API route、表单回填和样式。
- 验证记录：
  - `npm test` 通过 55 项。
  - `npm run lint` 通过。
  - `npm run build` 通过。
...
- 完成用户反馈：
  - 桌面端创建流程改为“实时预览在左，表单在右”，避免预览因为表单分栏掉到下方。
  - 手机端保持单列布局，并让实时预览置顶显示。
  - 表单左右字段统一进入表单列，实时预览固定在预览列。
- 验证记录：
  - `npm test` 通过 56 项。
  - `npm run lint` 通过。
  - `npm run build` 通过。
